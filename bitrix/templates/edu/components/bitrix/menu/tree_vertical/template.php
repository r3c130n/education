...
<?
	...
	foreach($arResult as $key => $arItem):
		...
		if ($previousLevel && $arItem["DEPTH_LEVEL"] < $previousLevel): // выходим из рекурсии
			echo str_repeat("</ul></li>", ($previousLevel - $arItem["DEPTH_LEVEL"]));
		endif;

		if ($arItem["IS_PARENT"]): //есть подкаталоги
			$i = $key;
...
?>
		<? if ($arItem['DEPTH_LEVEL'] > 1 && !$childSelected && $bHasSelected) :  // если выбран подкаталог, но не детки?>
...
<?
		else:  // нет подкаталогов
			if ($arItem["PERMISSION"] > "D"):
				$className = $arItem['SELECTED'] ? 'current selected' : '';
			/*if ($arItem['DEPTH_LEVEL'] > 1)*/ $className.= " lvl".$arItem['DEPTH_LEVEL'];
?>
			<li<?=$className ? ' class="'.$className.'"' : ''?>>
				<span style="position:relative; display: block;"><a href="<?=$arItem["LINK"]?>"><?=$arItem["TEXT"]?></a></span>
			</li>
<?
			endif;
		endif;
